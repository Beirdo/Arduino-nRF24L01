...
bool RF24::begin(void)
{
    ...
    // necessarily stick.  This is actually only required following power up
    // but some settling time also appears to be required after resets too.
    // For full coverage, we'll always assume the worst.
    //
    // Enabling 16b CRC is by far the most obvious case if the wrong timing is
    // used - or skipped.
    // Technically we require 4.5ms + 14us as a worst case. We'll just call it
    // 5ms for good measure.
    // WARNING: Delay is based on P-variant whereby non-P *may* requirei
    // different timing.
    delay(5);
    ...
    // or maximum packet sizes must never be used.
    // See documentation for a more complete explanation.
    setRetries(5, 15);
    ...
    // Enable PTX, do not write CE high so radio will remain in standby I
    // mode (130us max to transition to RX or TX instead of 1500us from
    // powerUp)
...
// Power up now. Radio will not power down unless instructed by MCU for
// config changes etc.
void RF24::powerUp(void)
{
    ...
    if (!(cfg & _BV(PWR_UP))) {
        ...
        // There must be a delay of Tpd2stby (see Table 16.) after the
        // nRF24L01+ leaves power down mode before the CE is set high.
        // - Tpd2stby can be up to 5ms per the 1.0 datasheet
        delay(5);
    }
}
...
bool RF24::write(const void *buf, uint8_t len, const bool multicast)
{
    ...
    if (status & _BV(MAX_RT)) {
        // Only going to be 1 packet in the FIFO at a time using this method,
        // so just flush
        flush_tx();
        return false;
    }
...
bool RF24::writeBlocking(const void *buf, uint8_t len, uint32_t timeout)
{
    ...
    while (get_status() & _BV(TX_FULL)) {
        // Blocking only if FIFO is full. This will loop and block until TX
        // is successful or timeout
...
bool RF24::writeFast(const void *buf, uint8_t len, const bool multicast)
{
    // Block until the FIFO is NOT full.
    // Keep track of the MAX retries and set auto-retry if seeing failures
    // Return 0 so the user can control the retries and set a timer or failure
    // counter if required
    // The radio will auto-clear everything in the FIFO as long as CE remains
    //  high

    uint32_t timer = millis();
    
    // Blocking only if FIFO is full. This will loop and block until TX isi
    // successful or fail
    while (get_status() & _BV(TX_FULL) ) {
        if (get_status() & _BV(MAX_RT)) {
            // Set re-transmit
            ...
            // From the user perspective, if you get a 0, just keep trying to
...
// Added the original startWrite back in so users can still use interrupts,i
// ack payloads, etc 
...
void RF24::whatHappened(bool& tx_ok,bool& tx_fail,bool& rx_ready)
{
    ...
    uint8_t status = write_register(NRF_STATUS, _BV(RX_DR) | _BV(TX_DS) |
                                                _BV(MAX_RT));
